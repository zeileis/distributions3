...
test_that("random.FisherF work correctly", {
  ...
  expect_length(random(s, rbind(1, 2, 3)), 3)
})
...
test_that("vectorization of a FisherF distribution work correctly", {
  ...
  r1 <- random(d)
  set.seed(123)
  r2 <- rf(2, df1 = c(5, 10), df2 = c(5, 10), ncp = 0)
  expect_equal(r1, r2)
...
test_that("named return values for FisherF distribution work correctly", {
  d <- FisherF(c(5, 10), c(10, 12))
