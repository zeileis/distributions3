...
test_that("random.ZTNegativeBinomial work correctly", {
  p <- ZTNegativeBinomial(mu = 1, theta = 1)
...
test_that("pdf.ZTNegativeBinomial work correctly", {
  p <- ZTNegativeBinomial(mu = 1, theta = 1)

  expect_equal(pdf(p, 0), dztnbinom(0, mu = 1, theta = 1))
  expect_equal(pdf(p, 1), dztnbinom(1, mu = 1, theta = 1))
...
test_that("log_pdf.ZTNegativeBinomial work correctly", {
  p <- ZTNegativeBinomial(mu = 1, theta = 1)

  expect_equal(log_pdf(p, 0), dztnbinom(0, mu = 1, theta = 1, log = TRUE))
  expect_equal(log_pdf(p, 1), dztnbinom(1, mu = 1, theta = 1, log = TRUE))
...
test_that("cdf.ZTNegativeBinomial work correctly", {
  p <- ZTNegativeBinomial(mu = 1, theta = 1)

  expect_equal(cdf(p, 0), pztnbinom(0, mu = 1, theta = 1))
  expect_equal(cdf(p, 1), pztnbinom(1, mu = 1, theta = 1))
...
test_that("quantile.ZTNegativeBinomial work correctly", {
  p <- ZTNegativeBinomial(mu = 1, theta = 1)

  expect_equal(quantile(p, 0), 1)
...
test_that("vectorization of a ZTNegativeBinomial distribution work correctly", {
  d <- ZTNegativeBinomial(mu = c(1, 2), theta = 1)
  d1 <- d[1]
  d2 <- d[2]
  ...
  ## elementwise
  expect_equal(
    pdf(d, c(1, 2), elementwise = TRUE),
    diag(pdf(d, c(1, 2), elementwise = FALSE))
  )
  expect_equal(
    cdf(d, c(1, 2), elementwise = TRUE),
    diag(cdf(d, c(1, 2), elementwise = FALSE))
  )
  expect_equal(
...
test_that("named return values for ZTNegativeBinomial distribution work correctly", {
  d <- ZTNegativeBinomial(mu = c(5, 10), theta = 1)
