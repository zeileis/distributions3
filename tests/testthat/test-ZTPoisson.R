...
test_that("pdf.ZTPoisson work correctly", {
  p <- ZTPoisson(1)

  expect_equal(pdf(p, 0), dztpois(0, 1))
  expect_equal(pdf(p, 1), dztpois(1, 1))
...
test_that("log_pdf.ZTPoisson work correctly", {
  p <- ZTPoisson(1)

  expect_equal(log_pdf(p, 0), dztpois(0, 1, log = TRUE))
  expect_equal(log_pdf(p, 1), dztpois(1, 1, log = TRUE))
...
test_that("cdf.ZTPoisson work correctly", {
  p <- ZTPoisson(1)

  expect_equal(cdf(p, 0), pztpois(0, 1))
  expect_equal(cdf(p, 1), pztpois(1, 1))
...
test_that("quantile.ZTPoisson work correctly", {
  p <- ZTPoisson(1)
...
test_that("vectorization of a ZTPoisson distribution work correctly", {
  d <- ZTPoisson(c(1, 2))
  d1 <- d[1]
  d2 <- d[2]
...
test_that("named return values for ZTPoisson distribution work correctly", {
  d <- ZTPoisson(c(5, 10))
